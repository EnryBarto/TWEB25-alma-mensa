...
if (isset($_GET["action"])) {
    ...
    switch($_GET["action"]) {
        ...
        case "U":
            ...
            break;
        case "D":
            if ($dbh->getReservationByCode($_GET["code"])->getUserEmail() != $user->getEmail()) {
                header("Location: explore.php");
                exit();
            }
            $exitCode = $dbh->deleteReservation($_GET["code"]);
            if ($exitCode == 0) {
                header("Location: manage_reservations.php?success=1");
                exit();
            } else {
                header("Location: manage_reservations.php?errorCode=$exitCode");
                exit();
            }
            break;
